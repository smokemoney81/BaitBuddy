...
export default function DashboardOverview({ user, nearestSpots = [], nextTrip = null, tripsError = false, onRetryTrips }) {
  ...
  // Die naechste Tour kommt aus der aggregierten Dashboard-Abfrage
  // (GET /api/dashboard). Vorher lud diese Komponente zusaetzlich die komplette
  // Trip-Liste und filterte clientseitig — der Aggregat-Endpunkt wurde also
  // abgerufen und sein Ergebnis bis auf top_spots verworfen, waehrend genau die
  // Einzelabfragen weiterliefen, die er ersetzen soll.
  ...
  return <>
    ...
    <BuddyCard message={upcoming} question={tripTitle ? `Hilf mir mit Wetter und Ausrüstung für meinen nächsten Trip ${tripTitle}.` : 'Welche Spots, Wetterbedingungen und Ausrüstung passen zu meinem nächsten Angelausflug?'}/>
    ...
    {tripsError ? <DataError onRetry={onRetryTrips}>Deine Trips konnten nicht geladen werden.</DataError> : <Link to="/TripPlanner" className="bb-card flex gap-4 items-center"><CalendarDays className="text-cyan-300"/><div className="flex-1"><h2 className="font-semibold">{tripTitle || 'Noch kein Angelausflug geplant'}</h2><p className="bb-muted">{tripDate ? new Date(tripDate).toLocaleString('de-DE') : 'Plane deinen ersten Ausflug mit deinem Buddy.'}</p></div><ArrowRight size={18}/></Link>}
  </>;
}
